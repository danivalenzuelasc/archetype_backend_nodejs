...
// Declare mocks spec
const mocks = require('./contact.mocks');
...
// Run testing
describe('Testing Endpoint /contact', async () => {
  /*  Testing method Create
   *  URI: /contact
   *  Method: POST
   */
  mocks.forEach(async (row) => {
    await test(`Testing ${getCounter()} - Method /contact (POST) [contact.create]`, async () => {
      counter += 1;
      await request().post('/contact').send(row)
        .then((response) => {
          if (response.statusCode === 201) {
            expect(response.body).toBeDefined();
            expect(response.body.active).toEqual(row.active);
            expect(response.body.business).toEqual(row.business);
            expect(response.body.details).toEqual(row.details);
            ...
            expect(response.body.notification).toEqual(row.notification);
            expect(response.body.phone).toEqual(row.phone);
            expect(response.body.position).toEqual(row.position);
   ...
   */
  mocks.forEach(async (row) => {
    await test(`Testing ${getCounter()} - Method /contact/:id (GET) [contact.view]`, async () => {
      await request().get(`/contact/${row._id}`)
        .then((response) => {
   ...
   */
  mocks.forEach(async (row) => {
    await test(`Testing ${getCounter()} - Method /contact/:id (PUT) [contact.update]`, async () => {
      const data = row;
      data.name += ' - Update';
      await request().put(`/contact/${row._id}`).send(data)
        .then((response) => {
          if (response.statusCode === 200) {
            expect(response.body).toBeDefined();
            expect(response.body.active).toEqual(row.active);
   ...
   */
  mocks.forEach(async (row) => {
    await test(`Testing ${getCounter()} - Method /contact/:id (DELETE) [contact.remove]`, async () => {
      await request().delete(`/contact/${row._id}`)
        .then((response) => {
          if (response.statusCode === 200) {
            expect(response.body).toBeDefined();
            expect(response.body.active).toEqual(row.active);
   ...
   *  Method: GET
   */
  test(`Testing ${getCounter()} - Method /contact (LIST) [contact.list]`, async () => {
    await request().get('/contact?limit=3&page=1&order=desc&logs=d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
  ...
  test(`Testing ${getCounter()} - Method /contact (LIST) [contact.list]`, async () => {
    await request().get('/contact?limit=5&page=2&order=asc&logs=c,d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
  ...
  test(`Testing ${getCounter()} - Method /contact (LIST) [contact.list]`, async () => {
    await request().get('/contact?short&limit=5&order=asc&logs=a,d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
  ...
  test(`Testing ${getCounter()} - Method /contact (LIST) [contact.list]`, async () => {
    await request().get('/contact?short&order=asc')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
  ...
  test(`Testing ${getCounter()} - Method /contact (LIST) [contact.list]`, async () => {
    await request().get('/contact?limit=-1&page=-1&order=asc')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
          expect(response.body.paging.count).toBeDefined();
   ...
   *  Method: GET
   */
  test(`Testing ${getCounter()} - Method /contact/search (SEARCH) [contact.search]`, async () => {
    await request().get('/contact/search?text=Name&limit=3&page=1&logs=d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
          expect(response.body.paging.count).toEqual(3);
  ...
  test(`Testing ${getCounter()} - Method /contact/search (SEARCH) [contact.search]`, async () => {
    await request().get('/contact/search?limit=-1&page=-1&logs=d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
          expect(response.body.paging.count).toEqual(0);
  ...
  test(`Testing ${getCounter()} - Method /contact/search (SEARCH) [contact.search]`, async () => {
    await request().get('/contact/search?text=Name&limit=4&logs=a,d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
  ...
  test(`Testing ${getCounter()} - Method /contact/search (SEARCH) [contact.search]`, async () => {
    await request().get('/contact/search?text=Name 01')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
          expect(response.body.paging.count).toBeDefined();
  ...
  test(`Testing ${getCounter()} - Method /contact/search (SEARCH) [contact.search]`, async () => {
    await request().get('/contact/search?text=01&logs=c,d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
          expect(response.body.paging.count).toBeDefined();
