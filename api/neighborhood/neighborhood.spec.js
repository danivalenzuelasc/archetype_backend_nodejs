...
// Declare mocks spec
const mocks = require('./neighborhood.mocks');
...
// Run testing
describe('Testing Endpoint /neighborhood', async () => {
  /*  Testing method Create
   *  URI: /neighborhood
   *  Method: POST
   */
  mocks.forEach(async (row) => {
    await test(`Testing ${getCounter()} - Method /neighborhood (POST) [neighborhood.create]`, async () => {
      counter += 1;
      await request().post('/neighborhood').send(row)
        .then((response) => {
   ...
   */
  mocks.forEach(async (row) => {
    await test(`Testing ${getCounter()} - Method /neighborhood/:id (GET) [neighborhood.view]`, async () => {
      await request().get(`/neighborhood/${row._id}`)
        .then((response) => {
   ...
   */
  mocks.forEach(async (row) => {
    await test(`Testing ${getCounter()} - Method /neighborhood/:id (PUT) [neighborhood.update]`, async () => {
      const data = row;
      data.name += ' - Update';
      await request().put(`/neighborhood/${row._id}`).send(data)
        .then((response) => {
          if (response.statusCode === 200) {
            expect(response.body).toBeDefined();
            expect(response.body.active).toEqual(row.active);
   ...
   */
  mocks.forEach(async (row) => {
    await test(`Testing ${getCounter()} - Method /neighborhood/:id (DELETE) [neighborhood.remove]`, async () => {
      await request().delete(`/neighborhood/${row._id}`)
        .then((response) => {
          if (response.statusCode === 200) {
            expect(response.body).toBeDefined();
            expect(response.body.active).toEqual(row.active);
   ...
   *  Method: GET
   */
  test(`Testing ${getCounter()} - Method /neighborhood (LIST) [neighborhood.list]`, async () => {
    await request().get('/neighborhood?limit=3&page=1&order=desc&logs=d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
  ...
  test(`Testing ${getCounter()} - Method /neighborhood (LIST) [neighborhood.list]`, async () => {
    await request().get('/neighborhood?limit=5&page=2&order=asc&logs=c,d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
  ...
  test(`Testing ${getCounter()} - Method /neighborhood (LIST) [neighborhood.list]`, async () => {
    await request().get('/neighborhood?short&limit=5&order=asc&logs=a,d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
  ...
  test(`Testing ${getCounter()} - Method /neighborhood (LIST) [neighborhood.list]`, async () => {
    await request().get('/neighborhood?short&order=asc')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
  ...
  test(`Testing ${getCounter()} - Method /neighborhood (LIST) [neighborhood.list]`, async () => {
    await request().get('/neighborhood?limit=-1&page=-1&order=asc')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
          expect(response.body.paging.count).toBeDefined();
   ...
   *  Method: GET
   */
  test(`Testing ${getCounter()} - Method /neighborhood/search (SEARCH) [neighborhood.search]`, async () => {
    await request().get('/neighborhood/search?text=Name&limit=3&page=1&logs=d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
          expect(response.body.paging.count).toEqual(3);
  ...
  test(`Testing ${getCounter()} - Method /neighborhood/search (SEARCH) [neighborhood.search]`, async () => {
    await request().get('/neighborhood/search?limit=-1&page=-1&logs=d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
          expect(response.body.paging.count).toEqual(0);
  ...
  test(`Testing ${getCounter()} - Method /neighborhood/search (SEARCH) [neighborhood.search]`, async () => {
    await request().get('/neighborhood/search?text=Name&limit=4&logs=a,d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
  ...
  test(`Testing ${getCounter()} - Method /neighborhood/search (SEARCH) [neighborhood.search]`, async () => {
    await request().get('/neighborhood/search?text=Name 01')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
          expect(response.body.paging.count).toBeDefined();
  ...
  test(`Testing ${getCounter()} - Method /neighborhood/search (SEARCH) [neighborhood.search]`, async () => {
    await request().get('/neighborhood/search?text=01&logs=c,d,t')
      .then((response) => {
        if (response.statusCode === 200) {
          expect(response.body).toBeDefined();
          expect(response.body.paging.count).toBeDefined();
